...
def append_engine(
    ...
    *,
    ...
    """Append an engine completion record to an existing manifest."""
    duration = (completed_at - started_at).total_seconds()
    record: dict[str, Any] = {
        "status":           "success",
        "version":          version,
    ...
    if extra:
        record.update(extra)
    manifest.setdefault("engines", {})[engine] = record

    errors = validate(manifest)
    if errors:
        raise ValueError("manifest validation failed after appending engine record:\n" + "\n".join(errors))
